...
fn main() -> io::Result<()> {
    ...
    match &File::open("sample/bird.bmp") {
        Ok(f) => println!("No issue  with the reference!"),
        Err(e) => println!("Error in the file open process! {:?}", e),
        _ => println!("Case fall-through"),
    }
